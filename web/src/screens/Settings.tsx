      ...
      <div className="grid grid-cols-[200px_1fr] gap-6">
        ...
        <div>
          ...
          <div className="overflow-hidden rounded-sheet border border-rule bg-sheet">
            <div className="grid grid-cols-[1.2fr_1fr_1fr_80px] gap-4 border-b border-rule px-4 py-2 text-13 text-ink-2">
              <span>Агент</span><span>Модел</span><span>Fallback</span><span>Буџет</span>
            </div>
            {routing && Object.entries(routing).map(([kind, r]) => (
              <div key={kind} className="grid grid-cols-[1.2fr_1fr_1fr_80px] gap-4 border-t border-rule px-4 py-2 text-13">
                <span>{KIND_LABEL[kind] ?? kind}</span>
                <span className="font-mono">{r.model}</span>
                <span className="font-mono text-ink-2">{r.fallback}</span>
                <span className="font-mono">${r.budgetUsd}</span>
              </div>
            ))}
          </div>
        </div>
