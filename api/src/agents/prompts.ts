...
// Local-audience framing — the reels are for a Macedonian (or Albanian) viewer,
// not a generic/translated one. Language ≠ market: this carries the mentality,
// buying psychology and register the text must hit (F2). Injected into every
// text-writing agent so it never drifts into translated "international" adspeak.
export function marketFraming(language: string): string {
  const mk = 'разговорен македонски како што реално се зборува (не преведено од англиски, не книжевно), локални референци и хумор каде е природно, психологија на купување на локалниот пазар (цена, доверба, препорака од близок)';
  const sq = 'разговорен албански како што реално се зборува (не преведено, не книжевно), локални референци и хумор каде е природно, психологија на купување на локалниот пазар (цена, доверба, препорака од близок)';
  const who =
    language === 'SQ'
      ? `АЛБАНСКИ гледач (Северна Македонија/дијаспора): ${sq}`
      : language === 'BOTH'
        ? `МАКЕДОНСКИ и АЛБАНСКИ гледач: ${mk}; за албанската верзија — ${sq}`
        : `МАКЕДОНСКИ гледач: ${mk}`;
  return `АУДИТОРИУМ — пишуваш за ${who}. Избегнувај калкиран рекламен англиски тон и генерички „интернационален“ глас.`;
}
